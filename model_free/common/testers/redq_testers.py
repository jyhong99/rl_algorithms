...
from model_free.baselines.policy_gradients.off_policy.redq.redq import redq  # noqa: E402
from model_free.baselines.policy_gradients.off_policy.redq.head import REDQHead  # noqa: E402
...
# Tests: REDQHead
# =============================================================================
def test_redq_head_actor_output_shape_and_bounds():
    """
    REDQHead uses a squashed Gaussian actor. We verify:
...
def test_redq_head_ensemble_sizes_and_q_shapes():
    """
    Validate critic ensemble sizes and Q output shapes.
    """
    seed_all(0)
    ...
    head = REDQHead(
        ...
        num_target_subset=num_subset,
    )

    assert_eq(len(head.critics), num_critics)
    assert_eq(len(head.critics_target), num_critics)

    obs = np.zeros((3,), dtype=np.float32)
    act = np.zeros((2,), dtype=np.float32)

    qs = head.q_values_all(obs, act)
    assert_eq(len(qs), num_critics)
    for qi in qs:
        assert_true(th.is_tensor(qi), "q_values_all should return tensors")
        assert_eq(tuple(qi.shape), (1, 1))

    qmin = head.q_values_target_subset_min(obs, act, subset_size=num_subset)
    assert_true(th.is_tensor(qmin), "q_values_target_subset_min should return a tensor")
    assert_eq(tuple(qmin.shape), (1, 1))


def test_redq_head_sample_action_and_logp_shapes():
    """
...
def test_redq_head_save_and_load_roundtrip():
    """
    Save/load roundtrip should reproduce actor + critic ensemble weights exactly.
    """
    seed_all(0)
    ...
    with tempfile.TemporaryDirectory() as td:
        path = os.path.join(td, "redq_head_ckpt")
        head1.save(path)
        ...
        # Critic ensemble compare
        assert_eq(len(head1.critics), len(head2.critics))
        for i, (q1, q2) in enumerate(zip(head1.critics, head2.critics)):
            c1 = q1.state_dict()
            c2 = q2.state_dict()
            assert_eq(set(c1.keys()), set(c2.keys()))
            for k in c1.keys():
                assert_true(th.allclose(c1[k], c2[k]), f"critic[{i}] mismatch after load: {k}")

        # Target ensemble compare (if saved)
        assert_eq(len(head1.critics_target), len(head2.critics_target))
        for i, (qt1, qt2) in enumerate(zip(head1.critics_target, head2.critics_target)):
            t1 = qt1.state_dict()
            t2 = qt2.state_dict()
            assert_eq(set(t1.keys()), set(t2.keys()))
            for k in t1.keys():
                assert_true(th.allclose(t1[k], t2[k]), f"target_critic[{i}] mismatch after load: {k}")


# =============================================================================
# Tests: OffPolicyAlgorithm integration (REDQ)
# =============================================================================
def test_redq_smoke_run_no_per_runs_and_updates():
    seed_all(0)
...
def test_redq_smoke_run_with_per_reports_beta():
    seed_all(0)
    env = DummyContinuousEnv(obs_dim=3, act_dim=2, horizon=50, seed=0, action_low=-1.0, action_high=1.0)
...
def test_redq_replay_batch_contract():
    """
...
def test_redq_parameters_change_after_update():
    """
    ...
    # For REDQ: critic ensemble is a ModuleList
    critics_before = th.cat([_state_vector(q) for q in algo.head.critics], dim=0)

    m = algo.update()
    assert_true(isinstance(m, dict) and len(m) > 0, "update() should return metrics")

    actor_after = _state_vector(algo.head.actor)
    critics_after = th.cat([_state_vector(q) for q in algo.head.critics], dim=0)

    da = float(th.linalg.vector_norm(actor_after - actor_before).item()) if actor_before.numel() else 0.0
    dc = float(th.linalg.vector_norm(critics_after - critics_before).item()) if critics_before.numel() else 0.0

    assert_true((da > 0.0) or (dc > 0.0), f"expected params to change; da={da}, dc={dc}")
    env.close()


def test_redq_stochastic_policy_non_deterministic_act_changes_actions():
    """
    REDQ uses a stochastic squashed Gaussian policy. With deterministic=False,
...
TESTS: List[Tuple[str, Callable[[], Any]]] = [
    ("redq_head_actor_output_shape_and_bounds", test_redq_head_actor_output_shape_and_bounds),
    ("redq_head_ensemble_sizes_and_q_shapes", test_redq_head_ensemble_sizes_and_q_shapes),
    ("redq_head_sample_action_and_logp_shapes", test_redq_head_sample_action_and_logp_shapes),
    ("redq_head_save_and_load_roundtrip", test_redq_head_save_and_load_roundtrip),

    ("redq_smoke_run_no_per_runs_and_updates", test_redq_smoke_run_no_per_runs_and_updates),
    ("redq_smoke_run_with_per_reports_beta", test_redq_smoke_run_with_per_reports_beta),
    ("redq_replay_batch_contract", test_redq_replay_batch_contract),
    ("redq_parameters_change_after_update", test_redq_parameters_change_after_update),
    ("redq_stochastic_policy_non_deterministic_act_changes_actions", test_redq_stochastic_policy_non_deterministic_act_changes_actions),
]


def main(argv=None) -> int:
    return run_tests(TESTS, argv=argv, suite_name="redq")
